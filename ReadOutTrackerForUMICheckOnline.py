...
class CSVLogger:
    def __init__(self):
        ...
        self.base_dir = "data"
        # 存储前一帧数据用于实时检查
        self.previous_data = {}  # {tracker_number: [timestamp, x, y, z]}
        # 检查阈值
        self.timestamp_threshold = 0.08  # 80ms
    ...
    def log_data_csv(self, tracker_number, current_time, position):
        """
        Log tracker data to CSV file and perform real-time consistency check.
        """
        try:
            if tracker_number in self.csv_writers:
                ...
                # 实时检查数据一致性
                self.check_real_time_consistency(tracker_number, frame_idx, current_time, position)
                ...
                # 更新前一帧数据
                self.previous_data[tracker_number] = [current_time, position[0], position[1], position[2]]
                
                self.frame_counters[tracker_number] += 1
        except Exception as e:
            print(f"Failed to write data to CSV file: {e}")

    def check_real_time_consistency(self, tracker_number, frame_idx, current_time, position):
        """
        实时检查数据一致性，发现问题立即打印
        """
        ...
        # 如果是第一帧数据，直接返回
        if tracker_number not in self.previous_data:
            return
        
        prev_data = self.previous_data[tracker_number]
        prev_timestamp = prev_data[0]
        prev_position = np.array([prev_data[1], prev_data[2], prev_data[3]])
        curr_position = np.array([position[0], position[1], position[2]])
        
        # 检查时间戳差值
        timestamp_diff = current_time - prev_timestamp
        if timestamp_diff > self.timestamp_threshold:
            print(f"⚠️  [TIMESTAMP WARNING] Tracker {tracker_number} ({tracker_name}) Frame {frame_idx}")
            print(f"   时间间隔过大: {timestamp_diff:.6f}s (阈值: {self.timestamp_threshold}s)")
            print(f"   前一帧时间: {prev_timestamp:.6f}")
            print(f"   当前帧时间: {current_time:.6f}")
            print()
        
        # 检查3D位置跳跃
        position_jump = np.linalg.norm(curr_position - prev_position)
        if position_jump > self.position_threshold:
            print(f"⚠️  [POSITION WARNING] Tracker {tracker_number} ({tracker_name}) Frame {frame_idx}")
            print(f"   位置跳跃过大: {position_jump:.6f}m (阈值: {self.position_threshold}m)")
            print(f"   前一帧位置: [{prev_position[0]:.4f}, {prev_position[1]:.4f}, {prev_position[2]:.4f}]")
            print(f"   当前帧位置: [{curr_position[0]:.4f}, {curr_position[1]:.4f}, {curr_position[2]:.4f}]")
            print()
...
def main():
    ...
    print_data = False # print normal data to console (disabled for cleaner output)
    real_time_check = True # enable real-time consistency checking

    print(f"配置:")
    print(f"  实时数据检查: {'启用' if real_time_check else '禁用'}")
    print(f"  时间戳阈值: {csv_logger.timestamp_threshold}s")
    print(f"  位置跳跃阈值: {csv_logger.position_threshold}m")
    print(f"  数据记录: {'启用' if log_data else '禁用'}")
            ...
            for i in range(openvr.k_unMaxTrackedDeviceCount):
                if poses[i].bPoseIsValid:
                    ...
                    if device_class == openvr.TrackedDeviceClass_GenericTracker:
                        ...
                        if log_data: 
                            # log_data_csv 现在包含实时检查功能
                            csv_logger.log_data_csv(tracker_number, current_time, position)
